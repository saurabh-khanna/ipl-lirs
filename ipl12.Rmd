---
title: "Gender Gap in Refugee Integration"
...
```{r}
# Libraries
pacman::p_load(tidyverse, haven, readxl, janitor, lubridate, extrafont, hrbrthemes, sf, ggcorrplot, plotly, ggradar, estimatr, texreg, estimatr, skimr, compareGroups, Hmisc)
extrafont::loadfonts()

# Parameters
df_baseline <- 
  read_csv("Data/LIRS/LIRS_baseline_data.csv") %>%
  inner_join(read_csv("Data/LIRS/LIRS_weights.csv"), by = "study_ID") %>% 
  ...
  mutate(
    ...
    householdAdults = householdSize - householdChildren
  )

df_asr2017 <-
  read_dta("Data/ASR_2017/ASR_2017_data.dta") %>% 
  mutate(
    years_in_us = 2018 - parse_number(as.character(qn1jyear)),
    female = if_else(qn1f == 2, 1L, 0L),
    employed = if_else(ui_emprate == 1, 1L, 0L),
    household_size = numppl,
    earnings = parse_number(as.character(ui_qn8a_annual)),
    earnings = if_else(earnings > 300000, NA_real_, earnings),
    public_assistance_received = if_else(ui_soi_pubassist == 1, 1L, 0L),
    cash_assistance_received = if_else(ui_cashassist == 1, 1L, 0L),
    medicaidrma_received = if_else(ui_medicaidrma == 1, 1L, 0L),
    legal_permanent_resident = if_else(ui_lpr == 1, 1L, 0L),
    baseline_school_years = parse_number(as.character(qn2a)),
    speak_english_well_now = if_else(qn4b <= 2, 1L, 0L),
    speak_english_well_baseline = if_else(qn4a <= 2, 1L, 0L)
  )

df_asr2018 <-
  read_dta("Data/ASR_2018/ASR_2018_data.dta") %>% 
  mutate(
    years_in_us = 2018 - parse_number(as.character(qn1jyear)),
    female = if_else(qn1f == 2, 1L, 0L),
    employed = if_else(ui_emprate == 1, 1L, 0L),
...
df_baseline %>% 
  ...
  skim() %>% 
  as_tibble() %>% 
  transmute(score = skim_variable, completion = round(complete_rate, 2), mean = numeric.mean, sd = numeric.sd, min = numeric.p0, max = numeric.p100) %>% 
  clean_names(case = "title") %>% 
  rename(SD = Sd) %>% 
  xtable::xtable()
```

Means (Weighted using sample weights):

```{r}
df_baseline %>% 
  select(contains("12s"), weight) %>% 
  summarize_at(vars(contains("12s")), ~ round(weighted.mean(., w = weight, na.rm = T), 2)) %>%
  # summarize_at(vars(contains("12s")), ~  round(sqrt(wtd.var(., weight)), 2)) #%>% 
...
```{r}
# preprocessing some vars for regressions
df_baseline <-
  df_baseline %>% 
  mutate(
    read_c = read,
...
Model: $$ipl12s = female + read + speak + householdSize + homeCountry + educationLevel + \epsilon$$
...
knitreg(list(lm1, lm2, lm3, lm4, lm5, lm6, lm7), include.ci = FALSE, custom.model.names = c("Overall", "Linguistic", "Political", "Social", "Economic", "Psychological", "Navigational"), stars = c(0.01, 0.05, 0.1), digits = 3, dcolumn = TRUE, booktabs = TRUE, float.pos = "H", caption = "", include.rsquared = F, include.adjrs = F, include.rmse = F, include.nclust = F)
```


### Years in US

Model: $$ipl12s = yearsInUS + \epsilon$$
...
### Years in US + Baseline characteristics

Model: $$ipl12s = yearsInUS + female + read + speak + householdSize + homeCountry + educationLevel + \epsilon$$
...
### Years in US + Baseline characteristics + State fixed effects

Model: $$ipl12s = yearsInUS + female + read + speak + householdSize + homeCountry + educationLevel + currentState + \epsilon$$
...
#### Household size $\times$ Years in US
...
#### Education terciles $\times$ Years in US
...
```{r}
df_baseline %>% count(gender, female)
```
...
Overall IPL-12 score by gender:
...
IPL-12 score components by gender:
...
#### By each IPL-12 question

```{r}
df_baseline %>% tabyl(connectionHc, female)
```

```{r}
df_baseline %>% tabyl(outsider, female)
```

```{r}
df_baseline %>% tabyl(polUnderstanding, female)
```

```{r}
df_baseline %>% tabyl(polDiscuss, female)
```

```{r}
df_baseline %>% tabyl(dinner, female)
```

```{r}
df_baseline %>% tabyl(contactsInUs, female)
```

```{r}
df_baseline %>% tabyl(read_c, female)
```

```{r}
df_baseline %>% tabyl(speak_c, female)
```

```{r}
df_baseline %>% tabyl(seeDoctor, female)
```

```{r}
df_baseline %>% tabyl(searchJob, female)
```
...
df_baseline <-
  ...
  mutate(
    ...
    citizen = recode(citizen, "No" = "0", "Yes" = "1", default = NA_character_) %>% as.integer(),
    cosponsorship = recode(cosponsorship, "No" = "0", "Yes" = "1", default = NA_character_) %>% as.integer(),
    educationLevelNumeric = if_else(educationLevel == "Received Master’s degree or higher", 7, educationLevelNumeric)
  )

df_baseline %>% tabyl(educationLevel, educationLevelNumeric)
```
...
```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(econ12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(ling12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(psy12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(pol12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female + age + houseHoldIncome + unemployment + educationLevelNumeric + citizen + cosponsorship, data = ., se_type = "stata", weights = weight)

knitreg(list(lm1, lm2, lm3, lm4, lm5, lm6, lm7), include.ci = FALSE, custom.model.names = c("Aggregate", "Economic", "Linguistic", "Social", "Psychological", "Political", "Navigational"), stars = c(0.01, 0.05, 0.1), digits = 3, dcolumn = TRUE, booktabs = TRUE, float.pos = "H", caption = "", include.rsquared = F, include.adjrs = F, include.rmse = F, include.nclust = F, omit.coef = "Intercept", custom.note = "%stars. All regressions use population-level survey weights generated from 9693 refugees resettled by the LIRS.", custom.coef.names = c("Female", "Age", "Household income", "Unemployed", "Education Level", "Citizenship", "Cosponsorship"))
```
...
```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(ling12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(pol12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(econ12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(psy12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female * years_in_us, data = ., se_type = "stata", weights = weight)

texreg(list(lm1, lm2, lm3, lm4, lm5, lm6, lm7), include.ci = FALSE, custom.model.names = c("Overall", "Ling", "Pol", "Soc", "Econ", "Psych", "Nav"), stars = c(0.01, 0.05, 0.1), digits = 3)
```


#### Female $\times$ Children in household

```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(ling12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(pol12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(econ12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(psy12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female * householdChildren, data = ., se_type = "stata", weights = weight)

texreg(list(lm1, lm2, lm3, lm4, lm5, lm6, lm7), include.ci = FALSE, custom.model.names = c("Overall", "Ling", "Pol", "Soc", "Econ", "Psych", "Nav"), stars = c(0.01, 0.05, 0.1), digits = 3)
```


#### Female $\times$ Household adults

```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(ling12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(pol12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(econ12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(psy12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female * householdAdults, data = ., se_type = "stata", weights = weight)

texreg(list(lm1, lm2, lm3, lm4, lm5, lm6, lm7), include.ci = FALSE, custom.model.names = c("Overall", "Ling", "Pol", "Soc", "Econ", "Psych", "Nav"), stars = c(0.01, 0.05, 0.1), digits = 3)
```


#### Female $\times$ Marital Status

```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(ling12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(pol12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(econ12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(psy12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female * maritalStatus, data = ., se_type = "stata", weights = weight)
...
```{r}
df_asr2017 %>%
  compareGroups::compareGroups(female ~ years_in_us + employed + earnings + public_assistance_received + cash_assistance_received + medicaidrma_received + household_size + legal_permanent_resident + baseline_school_years + speak_english_well_baseline + speak_english_well_now, data = .) %>% 
  compareGroups::createTable(show.all = T)
```
...
```{r}
df_baseline %>% 
  select(female, contains("12s")) %>% 
  drop_na(female) %>% 
  compareGroups::compareGroups(female ~ ipl12s + psy12s + econ12s + nav12s + soc12s + pol12s + ling12s, data = .) %>% 
  compareGroups::createTable() %>% 
  compareGroups::export2latex()
```
...
df_baseline %>%
  ...
  select(female, years_in_us, citizen, age, age_at_arrival, houseHoldIncome, unemployment, cosponsorship, householdChildren, householdSize, education = educationLevelNumeric, read, speak) %>% 
  drop_na(female) %>% 
  compareGroups(female ~ years_in_us + citizen + age + age_at_arrival + houseHoldIncome + unemployment + cosponsorship + householdChildren + householdSize + education + read + speak, data = .) %>% 
  createTable() %>% 
  export2latex()



df_baseline %>% select(contains("country")) %>% drop_na(bcountry) %>% count(bcountry) %>% rename(country_of_origin = bcountry) %>% arrange(-n) %>% view()
```
