---
title: "Refugee Integration Paper Results"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: yes
...
```{r}
# Libraries
pacman::p_load(tidyverse, haven, readxl, janitor, lubridate, extrafont, hrbrthemes, sf, ggcorrplot, plotly, ggradar, estimatr, texreg, estimatr)
extrafont::loadfonts()
...
df_asr2017 <-
  read_dta(here::here("data/ASR_2017", "ASR_2017_data.dta")) %>% 
  mutate(
...
df_asr2018 <-
  read_dta(here::here("data/ASR_2018", "ASR_2018_data.dta")) %>% 
  mutate(
    years_in_us = 2018 - parse_number(as.character(qn1jyear)),
    female = if_else(qn1f == 2, 1L, 0L),
    employed = if_else(ui_emprate == 1, 1L, 0L),
...
```{r, results='asis'}
lm1 <- df_baseline %>% lm_robust(ipl12s ~ female, data = ., se_type = "stata", weights = weight)

lm2 <- df_baseline %>% lm_robust(ling12s ~ female, data = ., se_type = "stata", weights = weight)

lm3 <- df_baseline %>% lm_robust(pol12s ~ female, data = ., se_type = "stata", weights = weight)

lm4 <- df_baseline %>% lm_robust(soc12s ~ female, data = ., se_type = "stata", weights = weight)

lm5 <- df_baseline %>% lm_robust(econ12s ~ female, data = ., se_type = "stata", weights = weight)

lm6 <- df_baseline %>% lm_robust(psy12s ~ female, data = ., se_type = "stata", weights = weight)

lm7 <- df_baseline %>% lm_robust(nav12s ~ female, data = ., se_type = "stata", weights = weight)
...
#round(sqrt((26.9^2 / 1032) + (27.93^2 / 1364)), 2)
```
