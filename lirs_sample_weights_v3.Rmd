...
```{r}
# Libraries
pacman::p_load(corrplot, caret)

# Population data
df_population <- 
  read.xlsx("data/Smartsheet_9.10.xlsx", sep.names = " ") %>% 
...
df_main <- 
  ...
  mutate(
    ...
    arrival_cy = arrival_cy %>% as_factor()
  ) %>%
...
```{r entropy}
eb.out <- ebalance(Treatment = df_main$treatment, X = df_main[ , 3:5])
...
sample_with_weights %>% 
  write_csv(here::here("sample_with_weights_09.25.2020.csv"))
