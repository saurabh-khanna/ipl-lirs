---
title: "LIRS Rapid Response Survey: September-November 2020 Follow-up Results"
...
# failed delivery numbers to exclude
failed_nos <- 
...
df_main <-
  ...
  mutate(
    ...
    wave = as.yearmon(wave, "%m-%Y") %>% as.character() %>% factor(levels = c("Sep 2020", "Oct 2020", "Nov 2020"))
  ) %>% 
...
df_main %>% 
  ...
  pivot_longer(cols = -wave, names_to = "name", values_to = "value") %>% 
  mutate(name = factor(name, levels = c("Survey started", "Survey completed", "Completed if started"))) %>% 
  filter(wave %in% c("Oct 2020", "Nov 2020")) %>% 
