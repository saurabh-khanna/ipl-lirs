---
title: "LIRS Followup Survey Report (Sep-Oct 2020)"
...
df_main %>%
  ...
  pivot_longer(cols = -wave, names_to = "name", values_to = "proportion") %>%
  full_join(
    df_main %>% 
      select(wave, survey_started, survey_completed) %>%
      group_by(wave) %>% 
...
### Q4 (Conditional on being unemployed)
...
### Q15 (Conditional on children in household)
...
### Q16 (Conditional on children in household)
